# 测试用例生成智能体 (PRD → Test Cases / Incremental Diff)
# 测试用例生成智能体 (PRD → Test Cases / Incremental Diff)

> 基于产品需求文档（PRD）自动生成结构化测试用例的原型项目。

## 📌 项目简介
本项目旨在提升软件测试准备阶段效率：测试/产品/开发仅需上传新版 PRD（或新旧版本差异文档）即可自动获得标准化的测试用例表格，支持增量模式、导出 CSV、回退模拟。
...
| `TEXT_MODEL_NAME` | 文本模型名称 | 优先级高于 `MODEL_NAME` |
| `VISION_MODEL_NAME` | 视觉模型名称 | 当前逻辑被 `DISABLE_VISION` 控制 |
| `MODEL_NAME` | 统一模型兜底 | 当未指定单独文本/视觉模型生效 |
| `DISABLE_VISION` | 是否禁用视觉 | 1=只文本；0=预留视觉路径 |
